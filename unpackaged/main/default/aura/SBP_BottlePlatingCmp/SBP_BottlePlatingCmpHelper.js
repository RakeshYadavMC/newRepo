({
    init: function (component, event, helper) {
        ...
        getRecordInfoPromise
            .then(
                $A.getCallback(function (result) {
                    console.log('record info ', result);
                    component.set('v.state', result['state']);
                    component.set('v.premiseType', result['premiseType']);
                    console.log('Premise Type---', result['premiseType']);
                    var retailAccountState = result['retailState'];
                    console.log('Retail Account State---', retailAccountState);
                    /*--- SR-168626 Start---*/
                    if (currentBrand == 'Knob Creek') {
                        if (retailAccountState == 'NY') {
                            component.set('v.checkNYOR', true);
                            component.set('v.labelText', 'Privately Selected');
                        } else if (retailAccountState == 'OR' && result['premiseType'] == 'On Premise') {
                            component.set('v.checkNYOR', true);
                            component.set('v.labelText', 'Privately Selected');
                        }
                    } /*--- SR-168626 End---*/ else if (currentBrand == 'El Tesoro') {
                        /*--- SR_183320 Start---*/
                        if (
                            result['state'] == 'NY' ||
                            result['state'] == 'New York' ||
                            ((result['state'] == 'OR' ||
                            result['state'] == 'Oregon') && result['premiseType'] == 'On Premise')
                        ) {
                            component.set('v.checkNYOR', true);
                            component.set('v.labelText', 'Privately Selected');
                        }
    ...
    validateBottlePlateText: function (component, event, helper) {
        ...
        //Added for state and city functionality
        var citystateText = component.get('v.citystateText');
        var bottlePlateValidityMap = component.get('v.bottlePlateValidityMap');

        if ($A.util.isEmpty(citystateText)) {
            bottlePlateValidityMap['Bottle_Plate_City_and_State'] = false;
        } else {
            bottlePlateValidityMap['Bottle_Plate_City_and_State'] = true;
        }
    ...
    removeBottlePlateTextFromMap: function (component, event, helper) {
        ...
        delete bottlePlateValidityMap['Bottle_Plate_City_and_State'];

        component.set('v.bottlePlateValidityMap', bottlePlateValidityMap);
    },
    defaultValidity: function (component, event, helper) {
        //alert('=======defaultValiditysd=====');
        var bottlePlateValidityMap = component.get('v.bottlePlateValidityMap');
        var bottle_plate = component.get('v.bottle_plate');
        bottlePlateValidityMap['bottle_plate'] = false;

        //Added for city and state functionality
        var include_City_and_State = component.get('v.include_City_and_State');
        bottlePlateValidityMap['include_City_and_State'] = false;
        component.set('v.bottlePlateValidityMap', bottlePlateValidityMap);
    },
    ...
    insertBottlePlateDetails: function (component, event, helper) {
        ...
        //alert('===pos items==='+JSON.stringify(posItems));
        console.log('json ', JSON.stringify(posItems));
        //alert('posItems to be sent ', JSON.stringify(posItems));
        action.setParams({
    ...
    addAccountDetailsToPosItem: function (component, event, helper) {
        ...
        //alert('addAccountDetailsToPosItem method'+JSON.stringify(selectedItemsMap));
    ...
    addRemovePosItem: function (component, event, helper, item, action) {
        var selectedItemsMap = component.get('v.selectedItemsMap');
        //alert('=========selectedmap under addremovepositem method=='+JSON.stringify(selectedItemsMap));
        ...
        if (action === 'INSERT') {
            //alert('====i m in insert====');
            //alert(selectedItemsMap.hasOwnProperty(item));
            //alert('===item val'+item);
            if (!selectedItemsMap.hasOwnProperty(item)) {
                //alert('I m in first insert looop');
                ...
                //alert('I m in second insert looop');
                selectedItemsMap[item]['Insert_Text__c'] = component.get('v.labelText');
                selectedItemsMap[item]['Type__c'] = item;
                selectedItemsMap[item]['Include_Signature_on_Label__c'] = signatureOnBackLabel;
                selectedItemsMap[item]['Include_Stave_Selection_on_Label__c'] = staveSelectionOnBackLabel;
    ...
    addBottlePlateText: function (component, event, helper) {
        var selectedItemsMap = component.get('v.selectedItemsMap');
        //alert('====I m in addBottlePlateText=selectedItemsMap=='+JSON.stringify(selectedItemsMap));
        ...
        //Added for city and state functionality
        if (selectedItemsMap.hasOwnProperty('Bottle Label Text City and State')) {
            selectedItemsMap['Bottle Label Text City and State']['Insert_Text__c'] = component.get('v.citystateText');
        }
        component.set('v.selectedItemsMap', selectedItemsMap);
    },
        ...
        if (!helper.isEmpty(runningTotalMap)) {
            ...
            for (var i in keys) {
                if (!shippingInfo) {
                    ...
                    bottlePlateRecordCity = {
                        Type__c: 'Bottle Plate City State',
                        Insert_Text__c: runningTotalMap[keys[i]].citystatetext,
                        Quantity__c: runningTotalMap[keys[i]].quantity,
                        Attention__c: component.get('v.attentionVal'),
                        Company__c: component.get('v.companyVal'),
                        Street__c: component.get('v.streetVal'),
                    ...
                    bottlePlateRecordCity = {
                        Type__c: 'Bottle Label Text City and State',
                        Insert_Text__c: runningTotalMap[keys[i]].citystatetext,
                        Quantity__c: runningTotalMap[keys[i]].quantity,
                        Attention__c: distributorRecord['Attention__c'],
                        Company__c: distributorRecord['Name'],
                ...
                labels.push(bottlePlateRecordCity);
            }
    ...
    validateAuraIdMap: function (component, event, helper) {
        //alert('=================i m in validateAuraIdMap=======');
    ...
    validateBottlePlateHasBeenEntered: function (component, event, helper) {
        //alert('validating bottle plate info');
        ...
        if (
            bottlePlatingOptionSelected &&
            (!showBottlePlating || splitLabel == 'Yes' || (showBottlePlating && labelText != '' && labelText != null))
        ) {
            return true;
        } else {
            if (bottlePlatingOptionSelected && showBottlePlating && (labelText == '' || labelText == null)) {
                if (
                    brand == 'Makers Mark' &&
                    (state == 'NY' || state == 'New York' || state == 'OR' || state == 'Oregon')
                ) {
                    return true;
                } else {
            ...
            if (!bottlePlatingOptionSelected) {
                helper.showToast(
                    'Please select whether or not you are ready to enter Bottle Label Details',
                    'Error',
                    'error'
                );
            }
    ...
    validateSplitLabels: function (component, event, helper) {
        //alert('validating split label info');
